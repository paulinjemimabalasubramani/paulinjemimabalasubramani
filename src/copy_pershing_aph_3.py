...
Copy Pershing APH files from remote location to source location
...
from modules3.common_functions import catch_error, data_settings, logger, mark_execution_end, relative_copy_file, collect_keys_from_config, find_latest_folder
...
def copy_files(firm:str, firm_remote_path:str):
    ...
    for file_name in os.listdir(firm_remote_path):
        ...
        if os.path.isfile(remote_file_path):
            ...
            try:
                source_type = file_name_noext.split('.')[1].upper()
            except:
                source_type = 'N/A'
...
def copy_files_per_firm():
    ...
    for firm, firm_remote_path in remote_paths.items():
        latest_remote_path = find_latest_folder(remote_path=firm_remote_path, folder_levels=folder_levels)
